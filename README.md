...
WinForms + SQLite 기반의 Windows 전용 로컬 이미지/영상 관리 뷰어 앱입니다. 파일을 앱 내부로 복사하지 않고 원본 경로를 SQLite DB에 저장해 관리합니다.
...
1. 앱을 실행합니다.
2. 상단 메뉴의 `설정`을 엽니다.
3. `이미지 메인` 또는 `영상 메인`으로 관리할 루트 폴더를 등록합니다.
4. 필요하면 `이미지 신규` 또는 `영상 신규`로 새로 받은 파일을 임시 등록할 폴더를 추가합니다.
...
- 이미지 메인/신규 루트: 이미지 폴더를 관리합니다.
- 영상 메인/신규 루트: 영상 파일을 관리합니다.
...
- Separate image mode and video mode
- Video-file based list management and external player launch
...
- Separate image/video main roots and new-registration roots
...
3. Add a library folder with `Image Main` or `Video Main`.
4. Optionally add a temporary incoming folder with `Image New` or `Video New`.
...
- Image main/new roots manage image folders.
- Video main/new roots manage video files.
